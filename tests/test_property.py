...
class TestFieldPropertyAliasesState:
    """gh-70: a ``[[properties]]`` entry with ``field = true`` and a name
    that matches an existing state field must NOT re-emit the struct member.
    Without the guard the struct ends up with duplicate fields and the C
    compiler errors with ``duplicate member``."""

    def test_no_duplicate_struct_field(self, tmp_path):
        dest = tmp_path / "dsp"
        new_run(
            "dsp",
            dest,
        ...
        struct_start = h.index("typedef struct {")
        struct_end = h.index("} reader_state_t;", struct_start)
        struct_body = h[struct_start:struct_end]
        assert struct_body.count("num_samples;") == 1, struct_body
        assert struct_body.count("position;") == 1, struct_body
