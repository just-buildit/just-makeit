...
| `--c-prefix P`                 | Prefix every C symbol jm derives with `P_` (`[project] c_prefix`): `fir_create` becomes `P_fir_create`, so two installed packages may share a component name. Never an author-named symbol or macro — see [c-library](../c-library.md#two-packages-one-program).                                                                                 |
