...
| `_csym.py`           | The one owner of the C symbol stem: every identifier jm derives from a component, module or function name (`<stem>_create`, `<stem>_state_t`, the `<STEM>_CORE_H` guard) starts with `stem(owner, name)`; the name stays the FILE stem. Templates read `<<csym>>`, which `render()` refuses without `slots(owner, name)`; a hand-spelled derived symbol is refused, and a stem-override render proves none escapes (gh-1591, gh-1633). `[project] c_prefix` (read by `_config.c_prefix`, applied only here) makes the stem `<p>_<name>` unless the name already starts `<p>_`; `duplicates` / `renames` / `unrenamed` read the replay's headers, so `apply` refuses two names deriving one symbol, and an existing tree whose author C still spells the unprefixed names |
