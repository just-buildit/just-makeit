...
from a Developer PowerShell or by opening its folder in Visual Studio: the
generated `CMakePresets.json` selects clang-cl, Ninja and the project's
`.venv` (gh-1376). CI builds every bundled example with clang-cl on
`windows-latest`, and that job gates the merge.

MSVC's own `cl.exe` cannot build it (it has no C99 `_Complex`), MinGW is
retired, and native ARM64 is not covered. The prerequisites, both ways in,
and what behaves differently are in
