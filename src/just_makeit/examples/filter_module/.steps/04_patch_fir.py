"""Patch my_filters_fir_step stub: drop const, add delay-line + convolution body."""
...
stub_re = re.compile(
    ...
    r"my_filters_fir_step\((?:const )?my_filters_fir_state_t \*state.*?\n\}",
    re.DOTALL,
)
m = stub_re.search(text)
if not m:
    print(
        "ERROR: my_filters_fir_step stub not found — already patched?",
        file=sys.stderr,
    )
    sys.exit(1)
