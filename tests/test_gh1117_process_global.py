...
class TestBranchesTheEndToEndDidNotReach:
    """Branches codecov flagged after gh-1117 phase 2 merged.

    The patch-coverage check failed on that PR and blocked nothing, so these
    are written from what it named rather than from a guess. One of them is a
    real gap and not a formality: every test in this file owns its state with
    a MODULE (doppler's shape), so the import path a STANDALONE owner
    produces had never been rendered once.
    """

    def test_a_standalone_owner_is_imported_by_its_own_name(self):
        """`p.flag`, not `p.<module>`.

        A standalone object's `.so` sits in the package root, so an adopter
        imports the component name directly. Every other test here owns via a
        module, where the path is the module's — so this spelling was
        ...
        assert _procglobal.owner_module(cfg, "flag") == "flag"
        assert _procglobal.import_path(cfg, "flag") == "p.flag"
        block = _procglobal.rendezvous_c(cfg, "other")
        assert 'PyImport_ImportModule("p.flag")' in block
        # ...and the owner still publishes rather than adopting from itself.
        assert "PyCapsule_New(flag_state_ptr()" in _procglobal.rendezvous_c(
            cfg, "flag"
        )
    ...
    def test_a_non_dict_top_level_section_is_skipped(self):
        ...
        manifest includes array-of-table sections a reader would never call
        components. Found by running the detector over doppler rather than
        over a fixture jm wrote itself — it raised `AttributeError` there."""
        cfg = {
            "project": {"name": "p"},
            "a": {},
            "weird": [{"name": "not-a-component"}],
        }
        cores = _procglobal.module_cores(cfg)
        assert "weird" not in cores
        assert _procglobal.shared_cores(cfg) == []

    def test_owner_of_something_that_is_not_a_component_is_none(self):
        assert (
            _procglobal.owner_module({"project": {"name": "p"}}, "nope")
            is None
        )

    def test_validate_skips_modules_that_do_not_link_the_core(self):
        """A `no_generate` module elsewhere in the project is not this
        component's problem, and refusing on it would make the feature
        unusable in any project that has one."""
        cfg = {
            "project": {"name": "p"},
            "flag": {"process_global": "true"},
            "module": {
                "own": {"objects": ["flag"]},
                "unrelated": {"kind": "handle", "no_generate": "true"},
            },
        }
        _procglobal.validate(cfg)
