...
| `_upgrade.py`        | `just-makeit upgrade` — schema migration for existing projects, and the schema-independent repairs: the `_Complex` respell (gh-1248) and the `c_prefix` respell (gh-1591: `_csym.renames` of a replay, rewritten code-only over gh-1583's `_project_files` walk, idempotent; gh-1653 adds the manifest's C-bearing values in place -- `*_impl`, `type`, an `*_impl_file`'s `::fn` whose file the walk respells -- and the `JM_DEFINE_STEPS` stem argument plus `<stem>_step_batch`, all read by `apply`'s refusal too)                                                                                                                                                                                                                                                                                                                                                   |
