...
| `--windows`                    | **Retired** (gh-1368): prints a notice and does nothing. A generated project builds on Windows with clang-cl as it is — see [Does it work on Windows?](../faq.md#does-it-work-on-windows).                                                                                                                                                       |
...
| `--find-package NAME`          | CMake `find_package(NAME REQUIRED)`. Repeatable. To name its pkg-config module too, write the entry as `{ name, pkg_config }` in `[project] find_packages` ([c-library](../c-library.md#when-your-library-depends-on-another-package)). See [`extra_link_libs`/module dependencies](../configuration.md#module-dependencies-external-libraries). |
| `--pkg-module NAME`            | pkg-config module linked via `pkg_check_modules`. Repeatable. May carry a version bound: `--pkg-module "zlib >= 1.2"`.                                                                                                                                                                                                                           |
| `--c-dep DIR`                  | Vendored C subdirectory under `native/src/DIR` with no Python wrapper (`[project] c_deps`). Repeatable.                                                                                                                                                                                                                                          |
| `--c-prefix P`                 | Prefix every C symbol jm derives with `P_` (`[project] c_prefix`): `my_dsp_fir_create` becomes `P_fir_create`, so two installed packages may share a component name. Never an author-named symbol or macro — see [c-library](../c-library.md#two-packages-one-program). **Default: the project name** -- `jm new dsp` derives `dsp_fir_create`.  |
| `--no-c-prefix`                | Derive bare C symbols (`my_dsp_fir_create`), as every project made before gh-1591 does.                                                                                                                                                                                                                                                          |
...
| `--header-only`                    | The component's C lives entirely in its header as `static inline` -- a macro template, or a family of inline functions. jm scaffolds **no `<comp>_core.c`**: the create/destroy/reset/steps/accessor bodies go into the sacred header instead, so the untouched scaffold still builds and passes before you replace them. The CMake core library becomes **INTERFACE** rather than OBJECT (an OBJECT library with no sources is a hard *configure* error), which also keeps it out of `lib<pkg>.so` -- there are no object files to fold in. Persists as `header_only = "true"`. |
