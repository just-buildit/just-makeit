...
def render_pyi(cfg: dict, module: str) -> str:
    ...
    lines = [
        ...
        "",
        "# Opaque handle (a PyCapsule); treat as an unintrospectable token.",
        f"{_to_state_alias(backing)} = Any",
        "",
    ]

    state_t = _to_state_alias(backing)
    ctor_args = ", ".join(
...
def _to_state_alias(backing: str) -> str:
    """A CamelCase alias name for the opaque state handle (``ddcr`` →
    ``DDCRState``)."""
    return f"{backing.upper()}State"
