...
| `--fragments`                  | Seed the project on the [per-component fragment layout](../declarative-scaffolding.md#three-layouts): objects route to `objects/<name>.toml` and modules to `modules/<name>.toml`, with the include globs pre-wired. Equivalent to running `jm migrate-to-fragments` immediately after `new`. |
