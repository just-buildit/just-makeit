...
| `_incpath.py`        | The one owner of the header layout: the `-I` directory (`INC_DIR`), where a project writes its own headers, and how an `#include` spells one (`include`, `core_include`); `render()` fills `<<inc_dir>>`/`<<inc_prefix>>` from it. `PREFIXED` is the gh-1583 layout switch; a test refuses a hand-spelled include root anywhere else |
