...
- **Example: upgrading an old project** (`stale_project`, gh-1443). A real
    project generated by just-makeit 0.33.14, with its author's kernel in the
    ...
    It is also the gate for the class of bug doppler kept finding first: a
    file an OLDER jm rendered meeting a guard added later, which a fresh
    scaffold cannot show. What every step prints is compared with a
    checked-in golden. A release that changes it on purpose regenerates the
    golden in the same PR, where the diff gets reviewed. The run first proves
...
- **Generated C now spells the complex types `_Complex`, not `complex`
    (gh-1246).** Same type, same ABI — `complex` is a `<complex.h>` macro for
    `_Complex`, so after preprocessing the tokens are identical and nothing
    about a project's behaviour changes. What changes is the text of every
    generated public header, and **an existing project is not fixed by
    upgrading jm.**

    Two files put it out of `apply`'s reach. `native/inc/clib_common.h` is
    create-only, so `apply` will not replace the `#define complex _Complex`
    shim it already has; `native/inc/<comp>/<comp>_core.h` is sacred, because
    it holds the author's inline `step()`, so nothing rewrites the spelling in
    it either. Measured on a project scaffolded before this change: `jm apply`,
    `jm regenerate <comp>` and `jm upgrade` all exit 0 and leave all 8
    occurrences in place.

    **Adopting only `clib_common.h` is worse than doing nothing.** That is the
    file `jm status --check` points at, and taking it alone removes the shim
    while leaving `float complex` in the prototypes — which trades gh-1246's
    failure for gh-1148's original one, `expected initializer before   '<comp>_step'`. Do both steps or neither:
    ...
    Verified end to end on a pre-change project: after both steps a C++11 TU
    using `std::complex` compiles, the project builds, and its CTest suite
    passes. `just-makeit` has no command that does this for you — that is
    gh-1248.

    You only need this if you compile the generated headers from C++.
    A C-only project is unaffected either way, though `jm status --check` will
    report `clib_common.h` as outdated until step 1 is taken.
...
- **A hand-written binding in `<mod>_ext_<obj>.c` no longer disappears on the
    next jm command (gh-770).** These fragments open with "Hand-patches to this
    file are preserved across jm commands". Three things made that untrue at
    once, and each alone was enough to lose the code.

    `_restore_c_function_bodies` only ever *replaced* a body whose name the
    fresh render also had, so a hand-**added** function — one the manifest
    cannot express, which is the entire reason these files are sacred — had
    nothing to match against and was written away. The `.pyi` has had the
    answer since gh-538 (a `# jm:hand` member with no manifest counterpart is
    appended rather than matched); the C side never grew the equivalent, and
    now has it: the function, the block comment above it, and the
    `PyMethodDef`/`PyGetSetDef` row that binds it all travel together. A
    carried function with no row is dead code; a carried row with no function
    is a link error.

    Separately, the body parser anchored on `name(`. GNU style — what
    `BasedOnStyle: GNU` gives, and what any project running clang-format over
    jm's C ends up with — writes `name (`, so extraction returned `{}`. **An
    empty extraction is not inert**: the caller read it as "nothing here to
    preserve" and overwrote the whole fragment, hand-edited bodies included.
    The gap before the paren is now optional whitespace, and the
    same-signature guard compares with whitespace removed rather than
    collapsed, so a formatted fragment still matches its unformatted render.
...
- **`jm apply` repairs a frozen `*_max_out` binding instead of only warning
    ...
    length-bearing `x_max_out(state, n)` gets `METH_VARARGS` — but existing
    fragments stayed frozen at whatever jm wrote when they were created. The
    `.pyi` moved to `max_out(n)`, the binding did not, and the call raised
    `TypeError`.

    Repaired for this class only, on the licence `_is_reclaimable_glue`
    already takes: a `*_max_out` wrapper has **no authoring path** — the
    implementation is in the sacred `_core.c` and the wrapper is marshalling
    nobody edits. Every other drifted binding stays a warning, because its
    body is the user's. The `METH_*` flags, the wrapper body and the runtime
    docstring move together: flags without the body is the one outcome worse
...
- **`jm apply` reports a refresh that would rewrite the constructor's keyword
    arguments (doppler#616).** Regeneration can change `kwlist[]` while losing
    no member at all, so `Obj(bank=…)` becomes a `TypeError` and `Obj(a, b)`
    binds its positionals to different parameters — invisible to any
    member-level audit, since both spellings still compile.

    Reported rather than repaired, deliberately: `<Obj>_init` is regenerated by
    design so a newly declared state field is never dropped from the
    constructor, and preserving the old `kwlist` under a fresh body would leave
    the name array and the `&var` list out of step —
    `PyArg_ParseTupleAndKeywords` would bind each keyword to the *neighbouring*
    variable. That compiles, runs, and puts the caller's values in the wrong
    fields, which is worse than either honest outcome.
