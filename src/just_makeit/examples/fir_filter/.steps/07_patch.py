...
# ── 2. Replace my_fir_fir_filter_steps() in source ──────────────────────────────────
...
if "JM_DEFINE_STEPS" in ctext:
    ...
    fn_re = re.compile(
        r"void\s+my_fir_fir_filter_steps\(.*?\n\}",
        re.DOTALL,
    )
    if not fn_re.search(ctext):
        print(
            "ERROR: my_fir_fir_filter_steps not found — already patched or file changed",
            file=sys.stderr,
        )
        sys.exit(1)
