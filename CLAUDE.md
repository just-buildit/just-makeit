...
| `_adopt.py`          | `just-makeit adopt --check` — would a module object's binding fragment be safe as jm's content? Per-fragment verdict from `_docsync.fragment_unit_diff`; read-only half of gh-1448                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `_upgrade.py`        | `just-makeit upgrade` — schema migration for existing projects, and the schema-independent repairs: the `_Complex` respell (gh-1248) and the `c_prefix` respell (gh-1591: `_csym.renames` of a replay, rewritten code-only over gh-1583's `_project_files` walk, idempotent; gh-1653 adds the manifest's C-bearing values in place -- `*_impl`, `type`, an `*_impl_file`'s `::fn` whose file the walk respells -- and the `JM_DEFINE_STEPS` stem argument plus `<stem>_step_batch`, all read by `apply`'s refusal too). Every C respell also passes the manifest through the ONE walker, `_csym.respell_manifest_c` (gh-1647: the `_Complex` respell reaches `*_impl` bodies), and `tests/test_gh1647_one_manifest_walker.py` refuses a C respell that does not                                                                                                          |
...
| `_recorddecl.py`     | `just-makeit record` — names a C struct and its columns ONCE per component, for both directions to reference (gh-1405)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `_enumc.py`          | One emitter for "validate a choice string to its `[[enum]]` int" — the lookup, the tables, the namespacing and the refusal, shared by every face (gh-1026)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `_borrow.py`         | A method that returns a zero-copy view of memory the C state owns — the predicate, which param sizes it, why a declaration is refused (gh-1312), and what a NULL MEANS: signals, then a declared `status_fn` → exception table (gh-1418)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `_outbuf.py`         | Whether a `variable_output` method offers an `out=` buffer — one predicate for the binding and both `.pyi` writers, and the reason when it does not (gh-1079)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `_docgaps.py`        | The members whose docstring is only their name — what `jm status --docs` reports, and where each would read its text from (gh-1394)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `_ctorsig.py`        | The `create()` the sacred `_core.h` declares against the one the manifest renders — jm verifying what it injected (gh-1076); and, advisory, the header's `@code` example calling it with a different argument count (gh-1502)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `_modplatforms.py`   | `[module.X] platforms` — which platforms a module's extension is built on: the CMake guard for every module kind and the platform-guarded re-export in `__init__.py`, both from one file (gh-1463)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `_procglobal.py`     | Which component cores reach more than one EXTENSION module, and `process_global = true` — the cross-module rendezvous jm generates into every `PyInit_`; detector and emitter in one file (gh-1117), plus the contract header left behind when the key is dropped (gh-1142)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `_rootcmake.py`      | The root `CMakeLists.txt` template's fixes outside jm's managed blocks, one row each, and which of them the project's file lacks -- read as CMake commands so a formatter's layout is not a finding; `status` reports them as `ROOT CMAKE` (gh-1459, gh-1471)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `_installhistory.py` | GENERATED (`make install-history-update`): every command a released jm rendered in the root install section, so `adopt --packaging` can tell an older jm's command from an author's (gh-1589)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `_projversion.py`    | The generated copies of `[project] version` — which files carry one, and which of them disagree with the manifest (gh-1141)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `_extrahook.py`      | A hand-written `*_extra.c` / `*_prologue.c` that the generated `_ext.c` beside it does not `#include`, so nothing compiles it — advisory, because a `handle`/`capsule` module has no hook to wire (gh-1202)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `_invariants.py`     | The contract between two members that speak one declared element -- what holds on a scaffold stub versus what needs a real kernel, and the jm-owned test file it goes in (gh-1404)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `_linkcheck.py`      | The address of every function the binding calls, linked into each component's C test, so a function declared and never defined fails `make test` at link time naming it (gh-1361)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `_libwiring.py`      | Which component cores reach `lib<pkg>.so` / `.a`, and which an additional `[project.libraries]` library (gh-1600) — emitter and detector in one file (gh-981/gh-984)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `_csym.py`           | The one owner of the C symbol stem: every identifier jm derives from a component, module or function name (`<stem>_create`, `<stem>_state_t`, the `<STEM>_CORE_H` guard) starts with `stem(owner, name)`; the name stays the FILE stem. Templates read `<<csym>>`, which `render()` refuses without `slots(owner, name)`; a hand-spelled derived symbol is refused, and a stem-override render proves none escapes (gh-1591, gh-1633). `[project] c_prefix` (read by `_config.c_prefix`, applied only here; `jm new` defaults it to the package name via `_new.DEFAULT_C_PREFIX`, `--no-c-prefix` opts out) makes the stem `<p>_<name>` unless the name already starts `<p>_`; `duplicates` / `renames` / `unrenamed` read the replay's headers, so `apply` refuses two names deriving one symbol, and an existing tree whose author C still spells the unprefixed names |
...
From manifest schema `_incpath.PREFIXED_SCHEMA` on (gh-1583), every header lives under
