...
| `ORPHAN`      | A `native/inc/<comp>/<comp>_procglobal.h` on disk for a component that no longer declares `process_global` (gh-1142). The same `apply` that dropped the key stripped the rendezvous from every generated `PyInit_`, so the header — stamped `DO NOT EDIT` — now describes one that is not generated and names a publisher that does not publish. It still compiles; what breaks is a hand-written binding that follows it. jm deletes nothing: remove the file, or re-declare the key.                                                                            | yes       |
| `DOC`         | A manifest `doc` holds more than one paragraph (gh-1154). A manifest `doc` is a **summary**: the rest is dropped for a module function, and reflowed into prose for a method — which also duplicates the `Parameters`/`Returns` jm generates. jm does not merge an author's numpy block with its own; write the full docstring as Doxygen above the declaration in the component's `_core.h`, where `@brief`/`@param`/`@return`/`@code` render a complete numpy docstring, doctest included, and survive `apply`. Suppressible per entry with `status_allow`.     | yes       |
