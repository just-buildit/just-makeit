...
| `error` / `error_message` (on a method) | over an `int` `returns`, a non-zero rc raises `error` with `error_message` (the rc appended) instead of crossing as an int; undeclared message → `"<fn> failed"`. The `.pyi` documents it as a numpy `Raises` section, from the same pair the binding raises with (gh-565/gh-1111) |
