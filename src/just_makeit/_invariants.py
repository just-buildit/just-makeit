...
def file_for(root: Path, pkg: str, comp: str) -> Path:
    """Where the generated invariants live for *comp*."""
    return root / "src" / pkg / "tests" / f"test_{comp}_invariants.py"
...
def _input_face(
    ...
    """
    make = (
        f"np.zeros(4, dtype={dt})"
        if scalar
        else "np.zeros(4, dtype=_ELEM_DTYPE)"
    )
    return [
