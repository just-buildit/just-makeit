...
@pytest.mark.parametrize(
    "msvc, build_python, want",
    [
        (True, True, "MultiThreadedDLL"),
        # A C-only build links no Python, so the debug CRT is fine there.
        (True, False, ""),
        (False, True, ""),
    ],
    ids=["msvc-python", "msvc-c-only", "posix"],
)
def test_a_python_build_uses_the_release_crt(
    tmp_path, msvc, build_python, want
):
    """Measured in Visual Studio 2026 + clang-cl: the Debug preset failed to
    link on `python312_d.lib` without this, and linked and passed with it."""
    root = _project(tmp_path)
    top = (root / "CMakeLists.txt").read_text(encoding="utf-8")
    start = top.index("if(MSVC AND BUILD_PYTHON)")
    end = top.index("endif()", start) + len("endif()")
    script = root / "crt.cmake"
    script.write_text(
        f"set(MSVC {int(msvc)})\nset(BUILD_PYTHON {int(build_python)})\n"
        + top[start:end]
    ...
    assert r.returncode == 0, r.stderr
    assert r.stderr.strip().splitlines()[-1] == f"[{want}]"
    # And it must precede every target: CMake reads it when a target is made.
    assert start < top.index("add_library(")
