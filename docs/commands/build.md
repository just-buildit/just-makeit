...
| `ROOT CMAKE`  | A fix jm's root `CMakeLists.txt` template carries outside its marked blocks and the project's root file lacks (gh-1459, gh-1471): the combined library's `libm` link (gh-1452), and the Windows build's static library name, DLL exports, `RUNTIME` install, default build type, release CRT, complex-range flag and CRT/math defines (gh-1368). Outside those blocks the file is yours, so `apply` never adds them; each line says what breaks without the fix and where, and `--diff` prints the file against today's render to merge from. Read as CMake commands rather than text, so a formatter's layout or your own equivalent spelling is not reported. Suppress one fix with `CMakeLists.txt:<fix>` in `status_allow`.                                               | no        |
