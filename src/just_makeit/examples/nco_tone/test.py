...
# latest release. Must be >= 0.14.1: 0.13.0 introduced the
# `doppler::doppler-static` cmake target this example links (and moved the
# install dir lib64/ -> lib/), and 0.14.x made the core C++-free (C99
# pocketfft, zmq/stream split out) so the static archive links with -lm
# alone — no -lstdc++, no g++ needed in the slim examples image.
_DOPPLER_VERSION = "0.14.1"
